...
int main()
{
    ...
    while(!run.empty())
    {
        ...
        {
            wait.push(make_pair(Process[proc].runtime, proc ));
            proc++;
        }
        int flag = 0;
        if(run.empty() && !wait.empty())
        {
            int ind2 = wait.top().second;
            flag = 1;
    ...
    for(i=0; i<n; i++)
    {
        cout<<"P"<<i+1<<"  - "<<Process[i].arrival<<" - "<<Process[i].start<<" - "<<Process[i].end<<" -  "<<Process[i].start - Process[i].arrival<<" - "<<Process[i].end - Process[i].arrival<<"\n";
        avg_wait += Process[i].start - Process[i].arrival;
